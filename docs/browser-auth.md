...
```sh
python -m pip install 'cmucw[browser]'
...
Canvas uses `CMUCW_CANVAS_TOKEN` if present; browser fallback occurs only if absent
...
No cookie export, credential file, or credential log is created by cmucw.
